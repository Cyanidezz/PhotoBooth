...
function selectFrame(item){config=item;resultId='';const overlay=$('liveFrame');overlay.onload=()=>document.querySelector('.live').style.aspectRatio=`${overlay.naturalWidth}/${overlay.naturalHeight}`;overlay.src=RemoteConfig.endpoint(`/api/frame/${item.id}.png`);overlay.hidden=false;$('result').hidden=true;$('preview').hidden=false;$('selection').hidden=true;$('booth').hidden=false;$('modeLabel').hidden=item.boxes.length!==3;socketSend({action:'reset',frame:item.id});status(connected?'พร้อมถ่าย':'กำลังเชื่อมต่อ Camera Server…');controls();}
